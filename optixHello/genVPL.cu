...
extern "C" __global__ void __closesthit__vpl_pos()
{
	...
	const vec3i index_mesh = sbtData.indices[primID];
	...
	// ------------------------------------------------------------------
	const float3 &A = sbtData.vertices[index_mesh.x];
	const float3 &B = sbtData.vertices[index_mesh.y];
	const float3 &C = sbtData.vertices[index_mesh.z];
	...
	const float3 diffuse_color = sbtData.diffuse_color;
	float3 ffnormal = Ns;
	...
	float3 thisContribution = prd.result * diffuse_color * nDl;//Compute its contribution
